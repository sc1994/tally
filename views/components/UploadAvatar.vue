...
.upload {
  ...
  .progress {
    ...
    span {
      ...
      height: 100%;
      width: 0%;
      border-bottom: 5px solid rgba(164, 208, 99, 0.8);
    }
  ...
  methods: {
        ...
        fr.onload = e => {
          ...
          this.img.onload = () => {
            ...
            if (this.img.naturalWidth > this.img.naturalHeight) {
              ...
              this.scaleHeight = Math.round(
                this.scaleWidth * this.img.naturalHeight / this.img.naturalWidth
              );
            }
            ...
            this.canvas.setAttribute(
              "style",
              "position:absolute;left:0;top:0;opacity:0;z-index:99999"
            );
    ...
    getFiles(e) {
      ...
      for (var i = 0; this.files[i]; i++) {
        if (
          this.files[i].size > this.limit ||
          !_mimes.includes(this.files[i].type)
        ) {
    ...
    cropDragMove(e) {
      ...
      if (this.isMobile) {
        if (e.touches.length === 2) {
          ...
          this.scale = Math.min(
            200,
            Math.max(100, this.scaleLength / this.beginLength * 100)
          );
          this.cropZoom();
        } else {
    ...
    cropZoom() {
      ...
      if (this.img.naturalWidth > this.img.naturalHeight) {
        ...
        this.scaleWidth =
          this.scaleHeight * this.img.naturalWidth / this.img.naturalHeight;
      } else {
    ...
    // 限制边界
    cropLimit() {
      if (this.scaleX < -(this.scaleWidth - this.width))
        this.scaleX = -(this.scaleWidth - this.width);
      if (this.scaleY < -(this.scaleHeight - this.height))
        this.scaleY = -(this.scaleHeight - this.height);
